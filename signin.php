<?php
    ...
    //$enc_pass = md5($p_asswd); //  Para un ejercicio se comentaréo el metodo diferente a md5 y se usa md5
                                //  Para la entrega final se debe usar password_hash
    
    //  Consulta (Query)
    $sql_login = "
        select u.id, u.email, u.firstname || ' ' || u.lastname as fullname, u.psswd from users u
        where u.email = '$e_mail'
    ";  //and u.psswd = '$enc_pass'
    ...
    if($res) {
        ...
        if($num > 0){
            if(!password_verify($p_asswd, $row['psswd'])) {
